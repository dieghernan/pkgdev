...
for (i in ncols) {
  rs_bk <- gsub(
    tb$dark_spec[i],
    tb$oksolar_dark_spec[i],
    rs_bk,
    ignore.case = TRUE
  )
}

rs_bk <- gsub("Selenized Dark", "OKSolar Dark", rs_bk, fixed = TRUE)
rs_bk <- gsub("selenized_dark", "oksolar_dark", rs_bk, fixed = TRUE)
rs_bk == rs
...
for (i in ncols) {
  rs_bk <- gsub(
    tb$dark_spec[i],
    tb$oksolar_light_spec[i],
    rs_bk,
    ignore.case = TRUE
  )
}

rs_bk <- gsub("Selenized Dark", "OkSolar Light", rs_bk, fixed = TRUE)
